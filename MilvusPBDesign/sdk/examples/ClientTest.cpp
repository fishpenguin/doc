...
std::vector<LeafQueryPtr<uint64_t>> GenLeafQuery() {
    ...
    std::string vector_query_param = "vector_qery_param";
    VectorQuery vq = {vector_field_name, vector_query_param, 100, 10, 1.0};

    std::vector<LeafQueryPtr<uint64_t>> lq;
    LeafQueryPtr<T> lq1 = std::make_shared<LeafQuery<uint64_t>>();
    LeafQueryPtr<T> lq2 = std::make_shared<LeafQuery<uint64_t>>()
    LeafQueryPtr<T> lq3 = std::make_shared<LeafQuery<uint64_t>>()
    lq.emplace_back(lq1);
    lq.emplace_back(lq2);
    lq.emplace_back(lq3);
...
_1_query_case() {
    ...
    //should
    auto shoud_clause = std::make_shared<BooleanClause<uint64_t>>(Occur::SHOULD);
...
_2_query_case() {
    ...
    auto shoud_clause1 = std::make_shared<BooleanClause<uint64_t>>(Occur::SHOULD);
    auto shoud_clause2 = std::make_shared<BooleanClause<uint64_t>>(Occur::SHOULD);
...
_3_query_case() {
    ...
    auto mmmnnn_res = proxy->Query(ssmn);
